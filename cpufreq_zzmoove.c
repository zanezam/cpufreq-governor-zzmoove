...
static inline unsigned int get_freq_target(struct zz_dbs_tuners *zz_tuners,
					   struct cpufreq_policy *policy)
{
	unsigned int freq_target = (zz_tuners->freq_step * policy->max) / 100;
...
static void zz_check_cpu(int cpu, unsigned int load)
{
	...
	if (load > zz_tuners->up_threshold) {
		...
		dbs_info->requested_freq += get_freq_target(zz_tuners, policy);
	...
	if (load < zz_tuners->down_threshold) {
		...
		freq_target = get_freq_target(zz_tuners, policy);
...
MODULE_DESCRIPTION("'cpufreq_zzmoove' - A dynamic cpufreq governor for "
